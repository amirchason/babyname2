  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-xl ${genderBg} border ${genderBorder}
                  ...
                  ${onClick ? 'cursor-pointer' : ''} group min-h-[240px] flex flex-col`}
    ...
    >
      ...
      <div className="relative p-3 pb-12 flex-1 flex flex-col">
        {/* Name - Properly sized */}
        <div className="mb-2 pt-6 text-center">
          <h3 className="text-2xl font-semibold text-gray-800 flex justify-center items-center gap-2 truncate px-2">
            {name.name}
            <span className={`text-2xl font-semibold bg-gradient-to-r ${genderColor}
        ...
        {/* Meaning - Properly sized with truncation */}
        {meaning && (
          <div className="mt-2 px-3 py-2 bg-white/30 rounded-lg border border-white/50 flex-1 flex items-center">
            <div className="flex items-center gap-2 justify-center w-full">
              <BookOpen className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" strokeWidth={1.5} />
              <span className="text-sm font-light text-gray-600 italic line-clamp-2 text-center">
                "{meaning}"
              </span>
            </div>
          </div>
        ...
        <div className="mt-3 flex items-center justify-center gap-2 flex-wrap">
          {(origin || name.origins) && (
            <div className="inline-flex items-center gap-1 px-2.5 py-1 bg-white/40 rounded-full border border-gray-200">
              <Globe className="w-3 h-3 text-gray-500" strokeWidth={1.5} />
              <span className="text-[10px] font-light text-gray-600 truncate max-w-[100px]">
                {name.origins ? name.origins.join(' • ') : origin}
              </span>
              {enriched && (
                <Sparkles className="w-2.5 h-2.5 text-gray-500 ml-0.5" strokeWidth={1.5} />
              )}
            </div>
          )}

          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-white/40 rounded-full border border-gray-200">
            <span className="text-[10px] font-light text-gray-600">Popularity</span>
            <span className={`text-sm font-semibold bg-gradient-to-r ${genderColor} bg-clip-text text-transparent`}>
