/**
 * V13 CLOUD ENRICHMENT SCRIPT - PARALLEL MODE
 *
 * Enriches top 1000 baby names with comprehensive v13 data using GPT-4o
 * Features:
 * - PARALLEL BATCH PROCESSING: 5 names at once (5x faster!)
...
const CONFIG = {
  ...
  MAX_RETRIES: 3,
  BATCH_SIZE: 5, // Process 5 names in parallel per batch
  TOP_N_NAMES: 1000,
...
// Load top N names from the CORRECT database
const loadTopNames = () => {
  log('Loading name database...', 'progress');

  try {
    // Use top100k_names.json which has the ACTUAL popular names (Muhammad #1, etc.)
    const dbPath = path.join(__dirname, '../public/data/top100k_names.json');
    const data = JSON.parse(fs.readFileSync(dbPath, 'utf-8'));
    const allNames = data.names;

    log(`Loaded ${allNames.length} names from top100k database`, 'info');

    // Sort by popularity rank and take top N
    const topNames = allNames
      .filter(n => n.popularityRank && n.popularityRank > 0 && n.name)
      .sort((a, b) => a.popularityRank - b.popularityRank)
      .slice(0, CONFIG.TOP_N_NAMES);

    log(`Selected top ${topNames.length} names (${topNames[0].name} #${topNames[0].popularityRank} → ${topNames[topNames.length-1].name} #${topNames[topNames.length-1].popularityRank})`, 'success');
    return topNames;
  } catch (error) {
...
const main = async () => {
  ...
  log(`🚀 PARALLEL MODE: Processing ${CONFIG.BATCH_SIZE} names per batch`, 'info');
  log('', 'info');

  for (let i = startIndex; i < namesToEnrich.length; i += CONFIG.BATCH_SIZE) {
    // Get batch of names
    const batch = namesToEnrich.slice(i, Math.min(i + CONFIG.BATCH_SIZE, namesToEnrich.length));
    const batchNumber = Math.floor(i / CONFIG.BATCH_SIZE) + 1;
    const totalBatches = Math.ceil((namesToEnrich.length - startIndex) / CONFIG.BATCH_SIZE);

    log(`📦 Batch ${batchNumber}/${totalBatches}: Processing ${batch.length} names in parallel...`, 'progress');

    // Process batch in parallel
    const results = await Promise.allSettled(
      batch.map(async (nameEntry) => {
        try {
          const enrichedData = await enrichName(nameEntry);
          return { success: true, name: nameEntry.name, data: enrichedData, nameEntry };
        } catch (error) {
          return { success: false, name: nameEntry.name, error: error.message, nameEntry };
        }
      })
    );
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      if (result.status === 'fulfilled' && result.value.success) {
        ...
        successCount++;
        log(`  ✅ ${nameEntry.name} (Rank #${nameEntry.popularityRank})`, 'success');
        ...
        log(`  ❌ ${nameEntry.name}: ${error}`, 'error');
    ...
    // Update manifest after batch
    saveManifest(manifest);

    // Update failed log
    saveFailedLog(failedLog);

    // Update checkpoint to end of batch
    const lastIndex = i + batch.length - 1;
    checkpoint.lastProcessedIndex = lastIndex;
    checkpoint.lastProcessedName = batch[batch.length - 1].name;
    checkpoint.totalProcessed += batch.length;
    saveCheckpoint(checkpoint);

    // Progress update
    const progress = ((lastIndex + 1) / namesToEnrich.length * 100).toFixed(1);
    log(`📊 Progress: ${lastIndex + 1}/${namesToEnrich.length} (${progress}%) - ${successCount} success, ${errorCount} errors`, 'info');
    log('', 'info');

    // Small delay between batches (not needed but adds safety margin)
    if (i + CONFIG.BATCH_SIZE < namesToEnrich.length) {
