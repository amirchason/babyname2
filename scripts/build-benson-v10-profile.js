...
console.log('🚀 Building Benson V10 Profile...\n');

// Load V10 data
console.log('📂 Loading V10 data...');
const v10Data = JSON.parse(fs.readFileSync('./public/data/enriched/benson-v10.json', 'utf-8'));
console.log(`✅ Loaded V10 data for: ${v10Data.name}`);

// Generate base profile
console.log('\n🎨 Generating base profile from V10 template...');
const baseHTML = generateNameProfile(v10Data);
console.log('✅ Base HTML generated');

// V10 post-processing: inject V10 features
console.log('\n✨ Adding V10 enhancements...');

function enhanceProfileWithV10Features(html, v10Data) {
  let enhanced = html;

  // 1. Add V10 badge
  const v10Badge = `
    <div style="display: inline-block; padding: 6px 12px; background: linear-gradient(135deg, #FF6B9D 0%, #C06C84 100%); color: white; border-radius: 12px; font-size: 11px; font-weight: 700; letter-spacing: 0.5px; margin-left: 12px; box-shadow: 0 2px 8px rgba(192, 108, 132, 0.3);">
      V10 PERFECT PROFILE
    </div>
`;
  ...
  if (v10Data.categories && v10Data.categories.length > 0) {
    console.log(`   🏷️  Categories: ${v10Data.categories.length} tags`);
    const categoryTagsHTML = v10Data.categories.map(cat => {
  ...
  // 3. Inject syllables
  if (v10Data.syllables) {
    console.log(`   🔢 Syllables: ${v10Data.syllables.count} (${v10Data.syllables.breakdown})`);
    const plural = v10Data.syllables.count > 1 ? 's' : '';
    const syllableHTML = `
  <p class="syllable-info" style="color: #64748b; font-size: 15px; margin: 8px 0 0 0; text-align: center;">
    ${v10Data.syllables.count} Syllable${plural}: <span style="color: #94a3b8;">${v10Data.syllables.breakdown}</span>
  </p>
`;
  ...
  if (v10Data.translations && v10Data.translations.length > 0) {
    console.log(`   🌍 Translations: ${v10Data.translations.length} languages`);

    const translationCards = v10Data.translations.map(trans => {
      const flagEmoji = {
        'Spanish': '🇪🇸',
        'Greek': '🇬🇷',
        'Arabic': '🇸🇦',
    ...
    <!-- Section 4.5: Name Around the World (V10) -->
    <div class="section">
...
`;
  ...
  const v10CSS = `
    <style>
      ...
      .category-american { background: linear-gradient(135deg, #FFEBEE 0%, #FFCDD2 100%) !important; color: #C62828 !important; }

      /* V10 Translation Cards */
      .translation-card {
...
const finalHTML = enhanceProfileWithV10Features(baseHTML, v10Data);
console.log('✅ V10 features integrated');

// Save
const outputPath = './public/benson-v10-profile.html';
fs.writeFileSync(outputPath, finalHTML);
console.log(`\n💾 Saved HTML to: ${outputPath}`);

console.log('\n✨ Benson V10 Profile Complete!');
console.log('   🌐 Open in browser to view');
console.log('   🎵 Features 3 positive-vibes-only songs!');
