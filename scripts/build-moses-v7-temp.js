 ...
 * Main build function
 */
async function buildMosesV7() {
  console.log('🚀 Building Moses V7 Profile...\n');

  // 1. Load V7 enriched data
  console.log('📂 Loading V7 data...');
  const v7DataPath = './public/data/enriched/moses-v7.json';
  if (!fs.existsSync(v7DataPath)) {
    console.error(`❌ Error: ${v7DataPath} not found!`);
    console.log('   Run: node scripts/enrich-v7-enhanced.js Moses');
    process.exit(1);
  }
  ...
  // 4. Save HTML profile
  const outputPath = './public/moses-v7-profile.html';
  fs.writeFileSync(outputPath, enhancedHTML);
  console.log(`\n💾 Saved HTML to: ${outputPath}`);

  console.log('\n✨ Moses V7 Profile Complete!');
  console.log('   🌐 Open with: termux-open public/moses-v7-profile.html');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  buildMosesV7().catch(console.error);
}
