/**
 * Audit Gender Information in Curated Lists V2
 * Checks ALL database sources (chunks 1-5, popularNames, ultimateNames)
 */
...
const popularNames = require('../public/data/popularNames_cache.json');
const ultimateNames = require('../public/data/ultimateNames_tier1.json');
...
allNames.forEach((name, index) => {
  ...
  if (!nameMap.has(nameLower)) {
    ...
    // Determine source
    let source = 'unknown';
    if (index < chunk1.length) source = 'chunk1';
    else if (index < chunk1.length + chunk2.length) source = 'chunk2';
    else if (index < chunk1.length + chunk2.length + chunk3.length) source = 'chunk3';
    else if (index < chunk1.length + chunk2.length + chunk3.length + chunk4.length) source = 'chunk4';
    else if (index < chunk1.length + chunk2.length + chunk3.length + chunk4.length + chunk5.length) source = 'chunk5';
    else if (index < chunk1.length + chunk2.length + chunk3.length + chunk4.length + chunk5.length + popularNames.length) source = 'popularNames';
    else source = 'ultimateNames';
...
console.log(`📊 CURATED LISTS GENDER AUDIT V2 (ALL SOURCES)`);
...
if (issues.notFound.length > 0) {
  ...
  console.log(`${'─'.repeat(80)}`);
  console.log(`First 50 missing names:`);
  issues.notFound.slice(0, 50).forEach(name => console.log(`  - ${name}`));
  if (issues.notFound.length > 50) {
    console.log(`  ... and ${issues.notFound.length - 50} more`);
  }
}
...
const reportPath = path.join(__dirname, 'curatedListsGenderAuditV2.json');
