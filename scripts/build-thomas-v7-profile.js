...
function enhanceProfileWithV7Features(htmlContent, v7Data) {
  ...
  if (v7Data.categories && v7Data.categories.length > 0) {
    ...
    // Insert after gender badge (find </div> after gender-badge)
    enhanced = enhanced.replace(
      /(<div class="gender-badge[^>]*>.*?<\/div>)/s,
      `$1\n${categoryTagsHTML}`
    );
  }

  // 2. Add Syllables to pronunciation line in Section 2
  if (v7Data.syllables) {
    const syllableHTML = `
      <span style="color: #94a3b8; margin: 0 8px;">•</span>
      <span class="syllable-info" style="color: #64748b; font-size: 15px;">
        ${v7Data.syllables.count} Syllable${v7Data.syllables.count > 1 ? 's' : ''}
        <span style="color: #94a3b8;">(${v7Data.syllables.breakdown})</span>
      </span>
    `;

    // Find pronunciation line and add syllables
    enhanced = enhanced.replace(
 ...
 * Main build function
 */
async function buildThomasV7() {
  console.log('🚀 Building Thomas V7 Profile...\n');

  // 1. Load V7 enriched data
  console.log('📂 Loading V7 data...');
  const v7DataPath = './public/data/enriched/thomas-v7.json';
  if (!fs.existsSync(v7DataPath)) {
    console.error(`❌ Error: ${v7DataPath} not found!`);
    console.log('   Run: node scripts/enrich-v7-enhanced.js Thomas');
    process.exit(1);
  }
  ...
  // 4. Save HTML profile
  const outputPath = './public/thomas-v7-profile.html';
  fs.writeFileSync(outputPath, enhancedHTML);
  console.log(`\n💾 Saved HTML to: ${outputPath}`);

  console.log('\n✨ Thomas V7 Profile Complete!');
  console.log('   🌐 Open with: termux-open public/thomas-v7-profile.html');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  buildThomasV7().catch(console.error);
}

export { buildThomasV7 };
